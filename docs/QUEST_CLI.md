...
- Forge refuses a missing or dirty Git baseline. Preserve wanted changes, stop any running Forge host, then use `npm run demo:reset -- confirm-reset` when an explicit clean reset is appropriate.
...
Codex authentication must already be available in the local Codex environment. The locked dependencies include the Codex CLI: use `npx codex login status`, then `npx codex login` if signed out. No credentials or downloaded Godot binaries are stored in the repository.
