...
export function RealPartDetailScreen({ busy, onBack, onEdit, onSnapshot, onStart, onTest, part, snapshot, state }: {
  busy: boolean;
  onBack: () => void;
  onEdit: () => void;
...
export function RealPartWorkflowScreen({ onBack, onFollowUp, onSnapshot, questId, snapshot }: {
  onBack: () => void;
  onFollowUp: (result: "needs_change" | "broken", note: string) => void;
  onSnapshot: (snapshot: GeneratedProjectWorldSnapshot) => void;
  questId: string;
  snapshot: GeneratedProjectWorldSnapshot;
}) {
  ...
  const submitResult = async (result: "worked" | "needs_change" | "broken" | "not_sure") => {
    ...
    await action(() => confirmGeneratedQuest(projectId, questId, mapped), result === "worked");
    if (result === "needs_change" || result === "broken") onFollowUp(result, feedback.trim());
  };
  ...
  return <section className="tool-screen real-workflow-screen"><header><button className="back-button-v3" disabled={run.phase === "implementing" || run.phase === "verifying" || run.phase === "completion_pending"} onClick={onBack} type="button"><Icon name="back" /> Back to Step</button><p className="screen-kicker">Building Step</p><h1>{quest.title}</h1><p>{run.contract.visibleOutcome}</p><StatusBadge status={run.phase === "completed" ? "complete" : run.phase === "failed" || run.phase === "scope_review" ? "attention" : "building"} /></header><div className="real-workflow-grid"><section className="detail-card"><h2>{phaseLabel[run.phase]}</h2><ol className="plain-progress">{steps.map((step) => <li className={step.done ? "done" : step.current ? "current" : ""} key={step.label}><span>{step.done ? <Icon name="check" /> : ""}</span>{step.label}</li>)}</ol>{run.progress.length > 0 && <p className="latest-progress">{run.progress.at(-1)}</p>}{notice && <p className="workflow-notice" role="status">{notice}</p>}{error && <p className="workflow-error" role="alert">{error}</p>}<div className="workflow-actions">{(run.phase === "contract_review" || run.phase === "approved") && <button className="forge-primary-button" disabled={busy} onClick={() => void continueWork()} type="button">{busy ? "Starting…" : "Continue Building"}</button>}{run.actions.play && <button className="forge-primary-button playtest-button" disabled={busy} onClick={() => void play()} type="button"><Icon name="play" /> Playtest</button>}{run.actions.confirm && played && <><label className="playtest-note"><span>What did you notice or want changed?</span><textarea onChange={(event) => setFeedback(event.target.value)} value={feedback} /></label><button className="forge-primary-button" disabled={busy} onClick={() => void submitResult("worked")} type="button">Worked</button><button className="forge-secondary-button" disabled={busy} onClick={() => void submitResult("needs_change")} type="button">Needs a Change</button><button className="forge-secondary-button" disabled={busy} onClick={() => void submitResult("broken")} type="button">Broken</button><button className="forge-secondary-button" disabled={busy} onClick={() => void submitResult("not_sure")} type="button">Not Sure</button></>}{run.actions.cancel && !run.actions.confirm && <button className="forge-secondary-button" disabled={busy} onClick={() => void action(() => cancelGeneratedQuest(projectId, questId))} type="button">Stop safely</button>}{run.actions.rollback && <button className="forge-secondary-button" disabled={busy} onClick={() => void action(() => rollbackGeneratedQuest(projectId, questId), true)} type="button">Undo reviewed changes</button>}{run.phase === "completed" && <button className="forge-primary-button" onClick={onBack} type="button">Return to World</button>}</div></section><aside className="detail-card workflow-proof"><p className="panel-kicker">Checks</p>{proofRows.map(([key, proof]) => <article key={key}><span className={"proof-dot " + proof.result} /><div><strong>{key === "projectHealth" ? "Godot health" : key === "boundary" ? "Chosen files" : key === "creator" ? "Your playtest" : "Extra check"}</strong><small>{proof.result.replaceAll("_", " ")}</small></div></article>)}<details className="advanced-details"><summary>Advanced Details</summary><p>Phase: <code>{run.phase}</code></p><p>Work ID: <code>{run.contract.fingerprint}</code></p><p>Allowed files:</p>{run.contract.allowedFiles.map((file) => <code className="contract-file" key={file.relativePath}>{file.relativePath}</code>)}{run.error && <p>{run.error}</p>}</details></aside></div></section>;
}
