...
export function friendlySystemPlanningError(message: string | null): string {
  if (!message || /invalid_json_schema|response_format|text\.format\.schema|invalid_request_error|\{\s*"type"\s*:/iu.test(message)) {
    return "Forge could not suggest systems this time. Your idea is still here, so you can try again safely.";
  }
  return message;
}
