...
export function SystemQuestRefinement({ projectId, systemId, systemTitle, systemOutcome, systemQuests, targetQuestId, onClose, onChanged, onReady }: {
  projectId: string;
  systemId: string;
  systemTitle: string;
  ...
  useEffect(() => {
    ...
    void loadSystemQuestPlanning(projectId, systemId, targetQuestId).then((value) => {
      if (active) { setSnapshot(value); setDescription(value.description); setBusy(false); }
    }).catch((next) => {
  ...
  }, [chosenQuest, fileChoiceOpen, files, recommendationAppliedQuestId]);
  ...
  const common = preparingQuest && chosenQuest
    ? <header><p className="v2-eyebrow">Prepare one quest · {systemTitle}</p><h1>{chosenQuest.title}</h1><p>{chosenQuest.playerVisibleOutcome}</p><small>No Godot file changes. Codex has not started.</small></header>
    : <header><p className="v2-eyebrow">Edit this game system</p><h1>{systemTitle}</h1><p>{systemOutcome}</p><small>No Godot file changes. Codex has not started.</small></header>;

  if (snapshot.phase === "idle" || (snapshot.phase === "cancelled" && !fileChoiceOpen)) return <main className="system-quest-refinement">{common}{error && <p className="workflow-error" role="alert">{error}</p>}<label><span>What should the player experience here?</span><textarea maxLength={1500} minLength={12} onChange={(event) => setDescription(event.target.value)} placeholder="The beacon should feel useful before the storm becomes dangerous…" rows={5} value={description} /></label><div className="system-quest-actions"><button className="v2-button button-quiet" onClick={onClose} type="button">Back to roadmap</button><button className="v2-button button-ember" disabled={busy || description.trim().length < 12} onClick={() => void run(() => startSystemQuestPlanning(projectId, systemId, description))} type="button">Suggest small quests</button></div></main>;
  ...
  if (snapshot.phase === "quests_accepted" && !fileChoiceOpen) return <main className="system-quest-refinement">{common}<section><h2>Quests saved for {systemTitle}</h2><p>Choose one quest to prepare. Forge recommends the next available step.</p><div className="workspace-quest-grid system-quest-proposal">{systemQuests.map((quest) => {
    const recommended = quest.questId === recommendedQuest?.questId;
    const actionable = quest.status === "available" && !quest.implementation;
    return <article className={`workspace-quest-card state-${quest.status}`} key={quest.questId}><small>{recommended ? "Recommended next · " : ""}{quest.status}</small><strong>{quest.title}</strong><p>{quest.playerVisibleOutcome}</p><button className={recommended ? "v2-button button-ember" : "v2-button button-quiet"} disabled={!actionable} onClick={() => { if (quest.workOrder) void onReady(quest.questId); else { setChosenQuestId(quest.questId); setFileChoiceOpen(true); } }} type="button">{quest.implementation ? "Completed" : quest.status !== "available" ? "Waiting for earlier quest" : quest.workOrder ? "Open quest" : "Prepare this quest"}</button></article>;
  })}</div></section>{error && <p className="workflow-error" role="alert">{error}</p>}<div className="system-quest-actions"><button className="v2-button button-quiet" onClick={leaveSavedQuests} type="button">Back to system</button></div></main>;
  ...
  if (snapshot.phase === "ready") return <main className="system-quest-refinement">{common}<section className="workspace-empty"><strong>Work plan saved.</strong><p>This quest and its chosen files are saved. Codex has not started and no Godot file changed.</p>{snapshot.workOrder && <code>{[...snapshot.workOrder.existingFiles, ...snapshot.workOrder.newFiles].join(" · ")}</code>}</section><button className="v2-button button-ember" disabled={!snapshot.workOrder} onClick={() => { if (snapshot.workOrder) void onReady(snapshot.workOrder.questId); }} type="button">Open quest</button></main>;

  if (!chosenQuest) return <main className="system-quest-refinement">{common}<p className="workflow-error" role="alert">Choose an available saved quest before selecting files.</p><button className="v2-button button-quiet" onClick={leaveSavedQuests} type="button">Back to system</button></main>;

  return <main className="system-quest-refinement">{common}<section><h2>Choose files for: {chosenQuest.title}</h2><p>{chosenQuest.playerVisibleOutcome}</p><details open><summary>Done when</summary><ul>{chosenQuest.doneWhen.map((item) => <li key={item}>{item}</li>)}</ul></details><div className="workspace-recommendation"><strong>Forge suggests starting here</strong><p>Suggestions are based on this quest and the game files already found. You can change them before review.</p>{recommendation.existingFiles.map((file) => <code className="contract-file" key={file}>{file}</code>)}{recommendation.suggestedNewFile && <button className="v2-button button-quiet" disabled={existingFiles.length + newFiles.length >= 4 || newFiles.includes(recommendation.suggestedNewFile)} onClick={() => setNewFileText((current) => [current.trim(), recommendation.suggestedNewFile].filter(Boolean).join("\n"))} type="button">Add suggested new file: {recommendation.suggestedNewFile}</button>}</div><fieldset><legend>Existing files</legend>{files.length ? files.map((file) => <label className="system-quest-file" key={file.relativePath}><input checked={existingFiles.includes(file.relativePath)} disabled={!existingFiles.includes(file.relativePath) && existingFiles.length + newFiles.length >= 4} onChange={(event) => setExistingFiles((current) => event.target.checked ? [...current, file.relativePath] : current.filter((item) => item !== file.relativePath))} type="checkbox" /><code>{file.relativePath}</code>{recommendation.existingFiles.includes(file.relativePath) && <small>Suggested</small>}</label>) : <p>No eligible existing files were found under scenes/ or scripts/.</p>}</fieldset><label><span>New files</span><textarea onChange={(event) => setNewFileText(event.target.value)} placeholder="scripts/welcome_beacon.gd" rows={3} value={newFileText} /><small>One path per line or comma. The parent folder must already exist.</small></label></section>{error && <p className="workflow-error" role="alert">{error}</p>}<div className="system-quest-actions"><button className="v2-button button-quiet" onClick={leaveSavedQuests} type="button">Back to system</button><button className="v2-button button-ember" disabled={busy || existingFiles.length + newFiles.length < 1 || existingFiles.length + newFiles.length > 4} onClick={() => void run(() => reviewSystemQuestWorkOrder(projectId, systemId, existingFiles, newFiles, chosenQuest.questId))} type="button">Check chosen files</button></div></main>;
}
