...
export function GeneratedProjectWorld({ initialSnapshot, onBack, onSnapshot }: {
  ...
  const center = systemQuestPlanningOpen
    ? <SystemQuestRefinement onChanged={refreshWorld} onClose={() => setSystemQuestPlanningOpen(false)} onReady={async (questId) => { setSystemQuestPlanningOpen(false); await persistView("quest_brief", questId); }} projectId={snapshot.project.projectId} systemId={presentation.selectedSystem.systemId} systemOutcome={presentation.selectedSystem.outcome} systemTitle={presentation.selectedSystem.title} />
    : systemPlanningOpen
    ...
    ? selectedQuest
      ...
      : <SystemQuestRefinement onChanged={refreshWorld} onClose={() => void closeBrief()} onReady={async (questId) => { setSystemQuestPlanningOpen(false); await persistView("quest_brief", questId); }} projectId={snapshot.project.projectId} systemId={presentation.selectedSystem.systemId} systemOutcome={presentation.selectedSystem.outcome} systemTitle={presentation.selectedSystem.title} />
    : active === "chronicle"
