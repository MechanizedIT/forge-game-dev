...
async function main() {
  ...
  try {
    ...
    await page.getByRole("heading", { name: "Review the exact new quests" }).waitFor();
    ...
    await page.getByRole("button", { name: "Accept exact quests" }).click();
    await page.getByRole("heading", { name: "Choose the first quest's exact file scope" }).waitFor();
    await capture(page, "file-choice-desktop");
    const firstFile = page.locator(".system-quest-file input").first();
    await firstFile.check();
    await page.getByLabel("Expected-new files").fill("scripts/welcome_beacon.gd");
    await page.getByRole("button", { name: "Review exact file scope" }).click();
    await page.getByRole("heading", { name: "Files proposed for the future exact contract" }).waitFor();
    await capture(page, "work-order-review-desktop");
    await page.getByRole("button", { name: "Accept exact work order" }).click();
    await page.getByText("Work-order draft saved for the next contract step.").waitFor();
